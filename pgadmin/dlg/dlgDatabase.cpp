...
void dlgDatabase::OnOK(wxCommandEvent &ev)
{
	...
	if (database)
	{
		...
		settings->Write(wxString::Format(wxT("Servers/%ld/Databases/%s/SchemaRestriction"), database->GetServer()->GetServerIndex(), database->GetName()), txtSchemaRestr->GetValue().Trim());
