...
pgObject *pgFunctionFactory::CreateObjects(pgCollection *collection, ctlTree *browser, const wxString &restr)
{
	...
	{
		funcRestriction = wxT(" WHERE prokind <> 'a' AND pronamespace = ") + NumToStr(collection->GetSchema()->GetOid())
		                  + wxT("::oid\n   AND typname NOT IN ('trigger', 'event_trigger') \n");
	}
	else
	{
		funcRestriction = wxT(" WHERE proisagg = FALSE AND pronamespace = ") + NumToStr(collection->GetSchema()->GetOid())
		                  + wxT("::oid\n   AND typname NOT IN ('trigger', 'event_trigger') \n");
	}
